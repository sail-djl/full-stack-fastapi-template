"""外汇数据查询和同步服务 - 从 forex schema 下的各个表查询和同步数据"""
import logging
import traceback
...
def _ensure_schema(session: Session) -> None:
    try:
        session.exec(text("CREATE SCHEMA IF NOT EXISTS forex"))
        session.commit()
...
class ForexService:
    ...
    def sync_from_tushare(
        ...
        for key, value in params.items():
            if value and isinstance(value, str) and ('start_date' in key or 'end_date' in key or 'trade_date' in key):
                if '-' in value:
