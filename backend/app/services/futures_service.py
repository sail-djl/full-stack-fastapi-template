"""期货数据查询和同步服务 - 从 futures schema 下的各个表查询和同步数据"""
import logging
import traceback
...
def _ensure_schema(session: Session) -> None:
    """确保futures schema存在"""
    try:
        session.exec(text("CREATE SCHEMA IF NOT EXISTS futures"))
        session.commit()
...
class FuturesService:
    ...
    def get_futures_list(
            ...
            for key, value in filters.items():
                if value is not None and value != '':
                    if key in ['start_date', 'end_date']:
                        ...
                        query += f" AND cal_date = :cal_date"
                        params["cal_date"] = value
                    elif key == 'is_open':
                        query += f" AND is_open = :is_open"
                        params["is_open"] = value
                    elif key == 'list_date':
                        # list_date是精确匹配
                        query += f" AND list_date >= :list_date"
                        params["list_date"] = value
                    else:
                        # 其他字段使用精确匹配
        ...
        if order_by:
            ...
            elif 'cal_date' in table:
                query += " ORDER BY cal_date DESC"
        ...
        try:
                ...
                for key, value in filters.items():
                    if value is not None and value != '':
                        if key in ['start_date', 'end_date']:
                            ...
                            count_query += f" AND cal_date = :cal_date"
                            count_params["cal_date"] = value
                        elif key == 'is_open':
                            count_query += f" AND is_open = :is_open"
                            count_params["is_open"] = value
                        elif key == 'list_date':
                            count_query += f" AND list_date >= :list_date"
                            count_params["list_date"] = value
                        else:
            ...
            if 'does not exist' in error_lower or 'relation' in error_lower or 'table' in error_lower:
                logger.error(f"Table {table} does not exist. Please create the table first by running the SQL scripts in 2-库表/7-期货/")
                # 返回更明确的错误信息
                return {
                    "data": [], 
                    "count": 0,
    ...
    def sync_from_tushare(
        ...
        Args:
            ...
            tushare_api: Tushare API方法名（如 "fut_basic"）
            params: Tushare API参数（可能包含需要转换的日期参数）
            date_columns: 需要转换的日期字段列表（如 ["list_date", "delist_date"]）
        ...
        for key, value in params.items():
            if value and isinstance(value, str) and ('start_date' in key or 'end_date' in key or 'list_date' in key or 'trade_date' in key or 'cal_date' in key):
